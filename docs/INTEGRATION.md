...
| 发送入口 C | Final Cut Pro 分享完成后通过 macOS 打开视频文件 |
...
| 400 | 路径不是普通文件 / 请求体非法 |
| 404 | 文件不存在；或未知路径 |
| 409 | 文件篮模式已开启，但文件篮功能未启用或文件无法加入 |
| 413 | 超过设置中的单文件上限；或请求体过大 |
| 503 | 微信未登录或上游失败 |
| 500 | 其它内部错误 |
...
对于短暂网络故障，App 会保留本次加密文件、`filekey`、AES 密钥和消息 `client_id`，在以下情况最多尝试 3 次（间隔 0.75 秒、1.5 秒）：

- HTTP `408`、`425`、`500`、`502`、`503`、`504`
- 请求超时、DNS 失败、无法连接、连接中断、当前无网络

CDN 上传重试会从文件开头重新上传，不会从已传字节位置继续。HTTP 4xx（上述 `408`、`425` 除外）、微信业务 `ret` 错误、响应字段缺失和数据解码错误不会自动重试。
...
本机 `/send` 会把这类微信上游错误返回为 HTTP `503`；错误 JSON 中可能同时包含微信上游的原始 HTTP 状态，例如“上传微信 CDN 文件失败：微信服务 HTTP 500”。
...
| 413 | 调整 App 的单文件上限，或压缩、切片文件 |
| 长时间无响应 | 可能在等待并发槽位、自动重试或上传大文件；查看 App 内具体阶段并加大超时 |
| 上游 HTTP 500/502/503/504 | App 会自动重试最多 3 次；仍失败时根据错误中的上传地址、CDN 上传或消息提交阶段排查 |
| 登录过期 | 重新扫码 |
