...
1. 先确认本机已安装 **Python 3.6+（64-bit）**。脚本是 Python 后渲染脚本，没有 Python 3 时可能无法运行。推荐安装最新稳定版 **Python 3.11 或 3.12**。
   ...
   **方式 B：官方安装包**
   ...
   也可在 App 设置页查看 Python 检测结果，并点「安装指南」。**App 不会安装或卸载系统 / Homebrew 的 Python**，只引导你手动处理，以免误清环境。

   **不需要 Python 时如何卸载（可选，自行操作）：**

   > 先确认你装的是 Homebrew 还是官网安装包。不要删除 macOS 自带的 `/usr/bin/python3`。
   ...
   **官网安装包安装的 Python：**
   ...
   1. 打开「应用程序」，把 `Python 3.x` 文件夹移到废纸篓  
   2. 在终端删除安装包留下的命令行工具（版本号按本机实际替换，例如 3.12）：

   ```bash
   sudo rm -rf /Library/Frameworks/Python.framework/Versions/3.12
   sudo rm -f /usr/local/bin/python3 /usr/local/bin/pip3
   ls /usr/local/bin | grep -Ei 'python|pip' || true
   python3 --version
   ```

   卸载 Python 不会删除 WeClaw Send 本身，也不会删除已安装的 DaVinci 脚本；若也不再使用脚本，请在设置页点「卸载」。
...
5. （可选）若只为 DaVinci 脚本安装了 Python，可按上文「不需要 Python 时如何卸载」自行卸载；**App 不会替你卸载 Python**
