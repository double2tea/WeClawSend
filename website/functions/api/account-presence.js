...
const SOURCES = new Set(["weClawSend", "openClaw"]);
const MAX_BODY_BYTES = 512;
const USER_AGENT = "WeClawSend-AccountPresence";
const PAYLOAD_KEYS = ["account_hash", "build", "channel", "source", "version"];
...
export async function onRequestPost(context) {
  const dataset = context.env.ACCOUNT_PRESENCE;
  ...
  const accountHash = String(payload?.account_hash || "");
  const version = String(payload?.version || "");
  const build = String(payload?.build || "");
  const channel = String(payload?.channel || "");
  const source = String(payload?.source || "");
  if (
    !HASH_PATTERN.test(accountHash)
  ...
  if (published === null) return textResponse("Release metadata unavailable", 503);
  if (!published) return textResponse("Unknown release", 400);

  try {
    dataset.writeDataPoint({
      blobs: [version, build, channel, source],
      doubles: [1],
      indexes: [accountHash],
    });
  } catch {
