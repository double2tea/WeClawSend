...
2. 如开启「API 文件先进入文件篮」，导出完成后文件会加入最近文件篮而不是立即上传微信。
3. 在 Premiere 打开一个序列。
4. 从面板内可搜索、可滚动的「我的预设」或「Premiere 系统预设」列表选择导出预设，再选择输出文件夹。
5. 选择「完整序列」或「入点到出点（I/O）」；I/O 未设置或范围无效时会在导出前提示。
6. 输入不含扩展名的文件名。面板重新获得焦点时会同步当前序列名；手动修改过的文件名不会被覆盖。
7. 按需开启「导出完成后自动发送」，然后点击导出。

关闭自动发送时，面板只导出；开启时，面板会先检查本机接口，导出成功后按 App 设置直接发送或加入文件篮。渲染完成后可立即切换序列继续导出；一个或多个发送失败时可逐个直接重试发送，不会重复渲染。导出文件不会覆盖同名文件，完成后可从面板在 Finder 中显示。导出由 Premiere 直接执行，不依赖 Adobe Media Encoder。
