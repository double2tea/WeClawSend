...
文件篮是类似 Dropover 的悬浮内容中转窗。你可以同时打开多个独立文件篮，收集文件、文件夹、macOS 包与主动粘贴的纯文本，再从当前篮批量发送到 ClawBot，或把项目拖回其它 App。外部文件只保存路径引用，不复制、不移动本地内容。

<p align="center">
  <img src="docs/images/file-basket-menu.png" width="820" alt="文件篮：多选项目，并通过菜单分享、压缩 ZIP、复制或在 Finder 中显示">
</p>

- 主面板文件篮按钮会打开统一管理菜单，显示文件篮和项目数量，并可新建、打开、显示全部、关闭全部、删除或切换重启恢复；右键菜单栏纸飞机也提供同一组文件篮信息和操作
- 刘海屏 Mac 默认启用「刘海投递区」：文件进入真实黑色刘海后，可在刘海内直接左右选择并松手投递，无需向下移入按钮；也可固定为「立即发送」或「放入文件篮」，此时只显示贴在刘海下缘的紧凑动作提示。放下后继续显示准备、上传进度、完成或失败；无刘海和外接普通屏不显示
- 默认 `⌥⌘S` 显示或隐藏最近使用的文件篮，可在设置中录入其它全局快捷键；拖着文件左右摇晃会在指针旁平滑展开新篮，松手时未放入文件则自动收起
- Finder 选中文件后，可从右键「服务」直接发送或放入最近文件篮；也可使用默认 `⇧⌘X` / `⇧⌘B`，并直接在 App 的文件篮设置中录入其它快捷键或关闭。快捷键首次使用时，允许 WeClaw Send 自动化控制 Finder；右键服务不需要该权限
- 每个文件篮拥有独立文件、名称、颜色、透明度、位置、折叠与置顶状态，默认命名为「文件篮 1、文件篮 2……」
- 默认以三列缩略图网格显示，可切换列表；图片、视频与文档会尽量显示 Quick Look 预览，无法生成预览时显示系统文件图标
- 文件篮获得焦点后可按 `⌘V` 粘贴文件、文件夹、图片或纯文本；文本保存为 App 托管的 `.txt` 便笺，剪贴板图片保存为独立 PNG 项目，均可继续预览、拖出或发送
- 窗口「…」菜单可新建文本便笺；单条文本上限 256 KiB，只有用户主动粘贴或新建时才读取和保存内容，不后台记录剪贴板
- 单选项目后可点“展开阅读”或按 `⌘Return`：TXT/Markdown 阅读器支持全量搜索高亮和前后跳转，PDF 支持连续滚动、缩放、页码、搜索及按住页面拖动平移，并可切换到文本选择模式；图片支持适应窗口或按解码像素显示（超大图会先缩小到 4096 像素）；视频和音频可预览、看波形、拖动进度并倍速播放；Office 与其它格式使用内嵌 Quick Look；阅读窗口默认 640×720，可从四边四角自由调整并按篮记住尺寸
- 文本编辑提供待办、编号和完成项整理；置顶提醒栏会把 `- [ ]` / `- [x]` 显示为可勾选项目，并在当前待办段内把完成项自动移到后方。提醒栏仍支持复制、编辑、立即发送、展开阅读或收回文件篮，可调整大小（最大约 760×500）
- 文件篮顶部“…”菜单及文件项目右键菜单均提供隔空投送、邮件和信息，可直接分享当前多选集合
- 支持拖入或选择文件、文件夹与 macOS 包；可从网格或列表空白处拖框选择，或用 `⌘A`、`⌘` 点选、`Shift` 点选后批量拖出、分享、复制、移除，也可 Quick Look 预览；清空或移除后可在 5 秒内撤销。删除文件篮不会删除 Finder 中的原文件，但会删除未再被引用的文本便笺和图片便笺
- 篮内仅含文件时保持立即发送；包含文件夹或包时，发送按钮会先要求命名，再压缩为单个 ZIP 发送
- 文件篮也支持延时发送；计划保存创建时的文件快照，创建计划后不会清空文件篮。含文件夹或包时会先压缩为 ZIP，再把 ZIP 加入计划
- 创建延时计划不要求当下已连接微信；到点时若微信离线、文件已缺失或发生变化，会进入“需要注意”，等待你确认后发送。应用退出期间错过时间，重启后也不会静默补发
- 窗口「…」菜单也可主动压缩 ZIP 发送、复制全部项目或路径，并在 Finder 中显示全部项目
- 默认保存文件篮内容、名称、外观、位置与窗口状态，并在 App 重启后恢复；可在文件篮管理菜单或设置中关闭
- 关闭空文件篮会自动删除；非空篮按「关闭时保留」设置隐藏保留或直接删除
- 批量发送只发送当前文件篮；发送后是否清空由设置控制
- 拖入菜单栏纸飞机图标仍然立即发送，不会进入文件篮
- 不支持符号链接、富文本混排或后台剪贴板历史；图片会作为独立文件项保存，不嵌入文本正文

## 文件夹监控

设置 → 监控可以在本地文件夹出现新文件后自动处理，不需要开启本地接口。可以添加多个文件夹，每个文件夹可建立多条类型分流，例如视频进入“成片篮”、图片进入“素材篮”、PDF 直接发送。

- 使用 macOS FSEvents 事件驱动，空闲时不轮询目录；默认静默等待 10 秒并再次检查大小、修改时间和文件占用，可改为 30/60 秒
- 添加规则前已存在的文件不会发送；移动进入或导出完成后新出现的文件会处理
- 默认忽略隐藏文件、下载临时文件、文件夹、macOS 包与符号链接
- 微信未连接时，直接发送规则会保留为等待任务，连接恢复后继续
- 同一规则在短时间内新增超过 20 个文件时，超出部分停止自动处理并提示，避免意外批量发送
- 监控只在 WeClaw Send 运行时生效；长期使用建议开启“登录时自动启动”

监控不会移动、改名或删除目标文件。进入文件篮时只保存原文件路径引用。
...
**Resolve 21 及更高版本注意事项：**使用脚本自动发送前，请关闭「偏好设置 → 后台任务 → 启用后台渲染」。开启后 `BgRenderManager` 回调可能缺少 Deliver `JobId`、状态和错误上下文，导致自动发送无法可靠工作。该问题已向 Blackmagic Design 提交（工单 `DEV-11860`），等待官方修复。
...
本地接口还支持创建、查看和管理延时发送计划。`POST /send` 默认保持立即发送，也可在设置中改为先加入最近文件篮，由用户确认后发送；计划接口不受此开关影响。
...
- 内测版：`WeClawReleaseChannel=beta`，标签为 `v<版本>-beta.<构建号>`（例如 `v2.4.0-beta.40`），并自动发布为 GitHub prerelease。
