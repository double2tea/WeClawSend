...
- 单选项目后可点“展开阅读”或按 `⌘Return`：TXT/Markdown 阅读器支持全量搜索高亮和前后跳转，PDF 支持连续滚动、缩放、页码、搜索及按住页面拖动平移，并可切换到文本选择模式；图片支持适应/原始尺寸，Office 与其它格式使用内嵌 Quick Look；阅读窗口默认 640×720，可从四边四角自由调整并按篮记住尺寸
- 文本编辑提供待办、编号和完成项整理；置顶提醒栏会把 `- [ ]` / `- [x]` 显示为可勾选项目，并在当前待办段内把完成项自动移到后方。提醒栏仍支持复制、编辑、发送、展开阅读或收回文件篮，尺寸可自由调整
- 文件篮顶部“…”菜单及文件项目右键菜单均提供隔空投送、邮件和信息，可直接分享当前多选集合
- 支持拖入或选择文件、文件夹与 macOS 包；可从网格或列表空白处拖框选择，或用 `⌘A`、`⌘` 点选、`Shift` 点选后批量拖出、分享、复制、移除，也可 Quick Look 预览；清空或移除后可在 5 秒内撤销，删除文件篮不会删除原内容
