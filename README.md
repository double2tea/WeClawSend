...
文件篮是类似 Dropover 的悬浮文件中转窗。你可以同时打开多个独立文件篮，先收集普通文件，再从当前篮批量发送到 ClawBot，或把文件拖回其它 App。它只保存文件引用，不复制、不移动本地文件。
...
- 支持拖入文件、选择文件、单项拖出、Quick Look 预览、移除单项、清空或彻底删除当前篮；删除文件篮不会删除原文件
- 窗口「…」菜单可先命名、再把当前篮压缩为单个 ZIP 并立即发送，也可复制全部文件路径或在 Finder 中显示全部文件
- 关闭空文件篮会自动删除；非空篮按「关闭时保留」设置隐藏保留或直接删除
- 批量发送只发送当前文件篮；发送后是否清空由设置控制
- 拖入菜单栏纸飞机图标仍然立即发送，不会进入文件篮
- 首版仅支持普通文件，不支持文件夹、文本或剪贴板内容
