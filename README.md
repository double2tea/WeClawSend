...
设置 → 集成中的“文件夹监控”可以在本地文件夹出现新文件后自动处理，不需要开启本地接口。可以添加多个文件夹，每条规则独立选择是否包含子文件夹、直接发送或进入文件篮，以及只处理视频、图片、音频、文档、压缩包或自定义扩展名。

- 使用 macOS FSEvents 事件驱动，空闲时不轮询目录；文件停止变化约 3–6 秒后再处理
