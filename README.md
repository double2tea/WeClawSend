...
本地接口还支持创建、查看和管理延时发送计划；`POST /send` 的行为保持不变，始终是立即发送。计划接口不要求创建时微信在线，计划会保存在本机并在应用运行时调度。
...
- 内测版：`WeClawReleaseChannel=beta`，标签为 `v<版本>-beta.<构建号>`（例如 `v2.3.0-beta.39`），并自动发布为 GitHub prerelease。
